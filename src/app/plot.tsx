...
const Plot = ({ data, tiempo, level }: {data: { t: number; h: number }[], tiempo: number, level: number}) => {
  return (
    <ResponsiveContainer
    ...
    >
      ...
      >
        ...
        <ReferenceLine y={level} strokeWidth={2} stroke="blue" label="nivel Deseado" />
...
export const Component = ({
  ...
  useAnimationFrame((time, delta) => {
    const newHeight = Math.round(ode(height, flow, delta / 100) * 1000) / 1000;
    ...
    <>
      <div className="h-[300px] relative">
        ...
        >
          ...
          <div className="w-20 bottom-0 left-full h-4 -translate-x-0 bg-gray-500 absolute">
            {/* <div className="h-5 shadow-lg w-2 top-1/2 -translate-y-1/2 bg-gray-400 right-0 absolute" /> */}
            <div className="h-5 shadow-lg w-2 top-1/2 -translate-y-1/2 bg-gray-400 left-0 absolute" />
...
export const Home = () => {
  const [flow, setFlow] = useState<number>(10);
  ...
  const handleAutoFixFlow = (error: number) => {
    setFlow((prevError) =>
      clampFunction(MIN_CAUDAL, MAX_CAUDAL,Math.round((prevError - error) * 1000) / 1000)
    );
  };
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="w-fit flex">
                <button 
                  ...
                  className={clsx("rounded-sm w-20 flex items-center justify-center px-2 py-1 rounded-l-xl font-bold",
                    mode == "AUTO" ? "bg-green-700 shadow-black/50   shadow-inner text-yellow-400" : "bg-green-600 shadow text-yellow-300"
                  )}>
                  Auto
                </button>
                <button
                  onClick={() => setMode("MANUAL")}
                  className={clsx("px-2 w-20 py-1 flex items-center justify-center rounded-r-xl font-bold",
                    mode == "MANUAL" ? "bg-green-700 shadow-black/50  shadow-inner  text-yellow-400 " : "bg-green-600 shadow text-yellow-300"
                  )}
                >
                  Manual
                </button>
